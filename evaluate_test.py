...
def run():
    ...
    parser.add_argument("--kp_method", type=str, default="cross_attention", help="{focus, cross_attention, concat_cross_attention, colbert}")
    ...
    with torch.no_grad():
        ...
        pg = Accuracy()
        pg_mtl = Accuracy(is_multilabel=True)
        kg = Accuracy()
        ...
        for test_data_index, test_data in enumerate(tqdm(test_loader)):
            ...
            # PG
            pg.update((p_index_cvtd, persona_grounding))
            pg_res = pg.compute()
            pg_mtl.update((p_index_cvtd, persona_grounding))
            pg_mtl_res = pg_mtl.compute()
